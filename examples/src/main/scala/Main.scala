...
import io.chrisdavenport.terminal.VirtualLineTerminal
...
object Main extends IOApp {
  ...
  def run(args: List[String]): IO[ExitCode] = for{
    _ <- IO.println("")
    _ <- (VirtualLineTerminal.impl[IO], EmberClientBuilder.default[IO].build).tupled.use{ case (vt, client) => 
      val resources = List(
      ...
      resources.parTraverse_{uri => 
        downloadAndShow(client, Request[IO](Method.GET, uri), vt, style = ProgressBarStyle.UNICODE_BLOCK)
      }
    }
  ...
  def downloadAndShow[F[_]: Temporal: Console](client: Client[F], req: Request[F], vt: VirtualLineTerminal[F], style: ProgressBarStyle = ProgressBarStyle.UNICODE_BLOCK): F[Response[F]] = {
    client.run(req).use{resp => 
      if (resp.status.isSuccess) {
        (
          Ref[F].of(0L),
          Temporal[F].realTime,
          resp.contentLength.toRight(new Throwable("No Content-Length")).liftTo[F],
          vt.newLine(Function.const(req.uri.toString()))
        ).tupled.flatMap{
          case (progress, start, length, token) => 
            resp.body.chunks.zipWithIndex
              .evalTap{ case (chunk, index) => 
                for {
                  ...
                  f = {(width: Int) => {
                    val base = StringUtils.padLeftZeros(
                      s" ${HumanReadable.decimalBased(currentLength)} / ${HumanReadable.decimalBased(length)} (${HumanReadable.decimalBased(rate)} / s)",
                      27,
                      ' '
                    )
                    val bar = DisplayBars.renderBar(width - base.length() - 1, percent, style)
                    val s = {
                      req.uri.toString().take(width) ++ "\n" ++
                      bar ++ base
                    }
                    fansi.Str(s)
                  }}
...
object StringUtils {
  def padLeftZeros(inputString: String, length: Int, char: Char): String = {
